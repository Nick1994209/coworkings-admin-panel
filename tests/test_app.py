...
def test_space_operations(authenticated_client):
    ...
    # Add a space
    rv = authenticated_client.post('/add_space', data=dict(
        name='Test Space',
    ...
    rv = authenticated_client.post('/update_occupancy/1', data=dict(
        occupancy=30
    ), follow_redirects=True)
    ...
    rv = authenticated_client.post('/add_equipment/1', data=dict(
    ...
    rv = authenticated_client.post('/edit_space/1', data=dict(
...
def test_registration_submission(authenticated_client):
    ...
    rv = authenticated_client.get('/registration_form')
    import re
    match = re.search(r'<option value="(\d+)">Registration Test Space \(Registration Test Location\)</option>', rv.data.decode('utf-8'))
    assert match, "Could not find space ID in registration form page"
    space_id = match.group(1)
    
    # Submit a registration
    rv = authenticated_client.post('/submit_registration', data=dict(
        firstName='John',
        lastName='Doe',
